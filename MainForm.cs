...
namespace WatchDog
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            this.Icon = WatchDog_Main.Properties.Resources.watchdog_16x16;
            this.notifyIcon1.Text = string.Format("{0} ({1})", CurExecName, CurExecVer);
            this.notifyIcon1.Visible = true;
            this.notifyIcon1.Icon = WatchDog_Main.Properties.Resources.watchdog_16x16;
        ...
        {
            PS PsXml = new PS();
            PsXml = DeserializeFromXml<PS>(XmlPath);
            DataTable dt1 = new DataTable();
            DataTable dt2 = new DataTable();
            List<ProcessItem> mp = new List<ProcessItem>();
            List<ProcessItem> sp = new List<ProcessItem>();
            if (File.Exists(XmlPath))
            {
                foreach (ProcessItem GItem in PsXml.SubProcess)
                {
                ...
                dt1 = ConvertToDataTable<ProcessItem>(sp);
                dataGridView1.DataSource = dt1;

                foreach (ProcessItem GItem in PsXml.MainProcess)
                {
                    if (File.Exists(GItem.Path))
                    {
                        mp.Add(GItem);
                ...
                dt2 = ConvertToDataTable<ProcessItem>(mp);
